...
import qr from '../../../public/assests/qr.png';
  ...
  <Document>
    <Page size="A4" style={styles.page}>
      ...
      <View style={styles.header}>
        <Image style={styles.qrTop} src={qr} />
        <Text style={styles.companyName}>PADMAVATI MARKETING</Text>
      ...
      <View style={styles.orderInfoSection}>
        <View style={styles.orderFrom}>
          ...
          <Text style={styles.customerInfo}>{order.userDetails.email}</Text>
          <Text style={styles.customerInfo}>{order.userDetails.phone}</Text>
          <Text style={styles.customerInfo}>{order.deliveryAddress.address}</Text>
          <Text style={styles.customerInfo}>{order.deliveryAddress.city || ''}</Text>
          <Text style={styles.customerInfo}>PIN: {order.deliveryAddress.pincode}</Text>
        ...
        <View style={styles.orderDetails}>
          ...
          <Text style={styles.orderNumber}>{order.orderId.slice(-2)}</Text>
          <Text style={styles.orderDate}>
            Date: {new Date(order.createdAt).toLocaleDateString('en-GB')}
          </Text>
          <Text style={styles.orderDate}>
            Due Date: {new Date(order.createdAt).toLocaleDateString('en-GB')}
          </Text>
        </View>
      </View>
      ...
      <View style={styles.itemsContainer}>
        {order.items.map((item, index) => (
          <View style={styles.itemCard} key={index}>
            ...
            <View style={styles.itemRow}>
              <Text style={styles.itemValue}>{item.quantity} Pac</Text>
              <Text style={styles.itemValue}>{formatPrice(item.price)}</Text>
              <Text style={styles.itemValue}>12.0%</Text>
      ...
      <View style={styles.pricingCard}>
        ...
        <View style={styles.transactionBalance}>
          ...
          <Text style={styles.balanceValue}>{formatPrice(order.totalAmount)}</Text>
        </View>
      </View>
...
export const generateInvoicePDF = async (order: Order) => {
  try {
    ...
    link.href = url;
    link.download = `sale-order-${order.orderId.slice(-8)}.pdf`;
